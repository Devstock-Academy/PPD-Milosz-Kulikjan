...
import { NextTaskIcon } from '@/icons'
...
export default function TasksList() {
  ...
  const { data: newTasks, isFetching } = useJsTasks(offset, limit)
        ...
        {tasksList.map((task: JsTask, index: number) => (
          ...
          >
            <Link href={`/${locale}/task/${task.id}`}>
              <button className='flex h-10 w-60 items-center justify-center gap-3 rounded-lg bg-activeSidebarBg'>
                {t('goToTask')}
                <NextTaskIcon />
              </button>
            </Link>
